...
THEORY ListConstantsX IS
  List_Valuable_Constants(Refinement(Game_ctx_r))==(DECK_TYPES,DECK_VALUES,DECK_VALUES2);
  Inherited_List_Constants(Refinement(Game_ctx_r))==(DECK_TYPES,DECK_VALUES,DECK_VALUES2);
...
THEORY ConstantsEnvX IS
  Constants(Refinement(Game_ctx_r)) == (Type(DECK_VALUES2) == Cst(SetOf(btype(INTEGER,?,?)*(etype(CARDS_TYPES,?,?)*etype(CARDS_VALUE,?,?))));Type(DECK_VALUES) == Cst(SetOf(btype(INTEGER,0,51)*etype(CARDS_VALUE,0,13)));Type(DECK_TYPES) == Cst(SetOf(btype(INTEGER,0,51)*etype(CARDS_TYPES,0,4))))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
